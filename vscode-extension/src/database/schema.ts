...
export class SchemaGenerator {
   ...
   */
  parseConceptFile(filePath: string): ConceptSpec {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (currentSection === 'state' && originalLine.startsWith('    ') && currentEntity && originalLine.includes(':')) {
        ...
        if (field) {
          const entityIndex = concept.state.findIndex((s: ConceptEntity) => s.name === currentEntity);
          if (entityIndex >= 0) {
  ...
  parseFieldDefinition(line: string): ConceptField | null {
    ...
    if (type.includes('|') && type.includes('"')) {
      enumValues = type.split('|').map((v: string) => v.trim().replace(/"/g, ''));
   ...
   */
  generatePrismaModel(concept: ConceptSpec): string {
    const models = [];
    const enums = [];
    ...
    for (const entity of concept.state) {
      ...
      const hasCreatedAt = entity.fields.some((f: ConceptField) => f.name === 'createdAt');
      const hasUpdatedAt = entity.fields.some((f: ConceptField) => f.name === 'updatedAt');
...
// SchemaGenerator is already exported above
